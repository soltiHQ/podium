...
func New(store storage.Storage, logger zerolog.Logger) *Service {
	if store == nil {
		panic("users.Service: store is nil")
	}
	return &Service{
...
func (s *Service) GetBySubject(ctx context.Context, subject string) (View, error) {
	if subject == "" {
		return View{}, storage.ErrInvalidArgument
	}
...
// Upsert a user.
func (s *Service) Upsert(ctx context.Context, model *model.User) error {
	return s.store.UpsertUser(ctx, model)
}
